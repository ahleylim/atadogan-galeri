...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // YENİ: Zoom ve Pan (Sürükleme) için elemanları ve değişkenleri seç
    ...
    // --- GitHub'dan Veri Çekme (Değişiklik Yok) ---
    fetch(apiUrl)
        .then(response => {
            if (response.status === 404) throw new Error(`'${imageFolderPath}' klasörü bulunamadı.`);
            if (!response.ok) throw new Error("GitHub API'den veriler alınamadı.");
        ...
        .then(data => {
            ...
            if (imageFiles.length === 0) {
                galleryContainer.innerHTML = `<p style="color: #ccc;">'${imageFolderPath}' klasöründe hiç resim bulunamadı.</p>`;
                return;
            }
    ...
    // GÜNCELLENDİ: Resim transformasyonunu (scale, translate) uygular
    function updateImageTransform() {
        lightboxImage.style.transform = `translate(${translateX}px, ${translateY}px) scale(${currentScale})`;
    }

    // GÜNCELLENDİ: Lightbox'ı açar ve tüm ayarları sıfırlar
    ...
    // GÜNCELLENDİ: Lightbox'ı kapatır ve ayarları sıfırlar
    ...
    // --- YENİ: Zoom (Yakınlaştırma) Eventleri ---
